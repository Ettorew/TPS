...
int compara_dupla(Show *a, Show *b, int *comparacoes) {
    (*comparacoes)++;
...
void shellSort(Show **shows, int n, int *comparacoes, int *movimentacoes) {
    for (int gap = n / 2; gap > 0; gap /= 2) {
        for (int i = gap; i < n; i++) {
            ...
            while (j >= gap && compara_dupla(shows[j - gap], temp, comparacoes) > 0) {
                shows[j] = shows[j - gap];
                j -= gap;
                (*movimentacoes)++;
            }
...
int main() {
    ...
    shellSort(meus_shows, k, &comparacoes, &movimentacoes);
    clock_t fim = clock();
