...
void quicksort(Show *arr[], int esq, int dir, int *comparacoes, int *movimentacoes) {
    ...
    while (i <= j) {
        while (comparar_shows(arr[i], pivo, comparacoes) < 0) i++;
        while (comparar_shows(arr[j], pivo, comparacoes) > 0) j--;
    ...
    if (esq < j) quicksort(arr, esq, j, comparacoes, movimentacoes);
    if (i < dir) quicksort(arr, i, dir, comparacoes, movimentacoes);
}
...
int main() {
    ...
    quicksort(meus_shows, 0, k - 1, &comparacoes, &movimentacoes);
    clock_t fim = clock();
