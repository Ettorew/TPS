...
class Show {
    ...
    public static int[] ordenaSelecao(Show[] arr) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            for (int j = i + 1; j < n; j++) {
                comp_mov[0]++;
                if (arr[j].title.compareTo(arr[minIdx].title) < 0) {
                    minIdx = j;
                }
            }
            comp_mov[1] += 3;
...
public class SelecaoParcial {
    public static void main(String args[]) throws FileNotFoundException, IOException {
        ...
        int[] comp_mov = Show.ordenaSelecao(meusShows);
        ...
        FileWriter fw = new FileWriter("matricula_selecaoparcial.txt");
