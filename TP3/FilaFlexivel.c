...
typedef struct {
    ...
    Celula *fim;
    int n;
    int capacidade; // Tamanho fixo da fila (como no modelo circular)
} FilaFlexivel;

// Criar fila
FilaFlexivel *newFila(int tam) {
...
// Remover do início
Show *remover(FilaFlexivel *fila) {
    if (fila->n == 0) {
        return NULL;
    }
    Celula *tmp = fila->inicio;
    Show *removido = tmp->dado;
...
// Inserir no fim
void inserir(FilaFlexivel *fila, Show *s) {
...
// Imprimir fila
void imprimirFila(FilaFlexivel *fila) {
...
// Média dos release_years
void mediaRY(FilaFlexivel *fila) {
    int soma = 0, count = 0;
    Celula *atual = fila->inicio;
    while (atual != NULL) {
        soma += atoi(atual->dado->release_year);
        count++;
        atual = atual->prox;
    }
